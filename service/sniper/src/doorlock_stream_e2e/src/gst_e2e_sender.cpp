...
namespace
{
  ...
  bool parse_args(int argc, char **argv, SenderConfig &cfg)
  {
    for (int i = 1; i < argc; ++i)
    {
      ...
      if (arg == "--mode")
      {
        if (!read_next(cfg.mode))
          return false;
      }
      else if (arg == "--file")
      {
        if (!read_next(cfg.file_path))
          return false;
      }
      else if (arg == "--camera")
      {
        if (!read_next(cfg.camera_device))
          return false;
      }
      else if (arg == "--host")
      {
        if (!read_next(cfg.host))
          return false;
      }
      else if (arg == "--port")
      {
      ...
      {
        std::string v;
        if (!read_next(v) || !parse_bool(v, cfg.loop))
          return false;
      }
      ...
      {
        std::string v;
        if (!read_next(v) || !parse_bool(v, cfg.enable_display))
          return false;
      }
  ...
  std::string build_pipeline(const SenderConfig &cfg)
  {
    ...
    ss
        ...
        // [GStreamer 内存契约防线]
        // drop 必须为 false！GStreamer 的 push_buffer 遵循 "Transfer Full" (完全移交) 规则。
        // 无论 push 成功还是失败，buffer 的生命周期已交给 appsrc 内部管理。
        // 绝对不能在 flow_ret != OK 时手动调用 gst_buffer_unref，否则会导致 Double Free 段错误崩溃。
...
int main(int argc, char **argv)
{
  ...
  while (!g_stop.load())
  {
    ...
    // [算法隔离] 将神经网络算出的坐标纯粹传递给预处理层，不在此处做任何图像平移(warpAffine)处理。
    ...
    while (true)
    {
      ...
      if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_WARNING)
      {
        ...
        if (dbg)
          std::cerr << "[sender][WARN] debug: " << dbg << std::endl;
        if (err)
          g_error_free(err);
        if (dbg)
          g_free(dbg);
      ...
      else if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_ERROR)
      {
        ...
        if (dbg)
          std::cerr << "[sender][ERROR] debug: " << dbg << std::endl;
        if (err)
          g_error_free(err);
        if (dbg)
          g_free(dbg);

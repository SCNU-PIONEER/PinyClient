...
namespace doorlock_stream_e2e
{
  namespace
  {
    ...
    std::string build_pipeline()
    {
      ...
      ss
          // [架构防线]
          // 必须设置为 false！如果设为 true，appsrc 会用系统时间覆盖 RTP 包内部的 32 位时间戳。
          // rtpjitterbuffer 完全依赖 RTP 内部时间戳来计算网络抖动和包乱序，
          // 外部强制打时间戳会导致 JitterBuffer 彻底混乱，引发积压数千包才出画的致命 Bug。
          << "appsrc name=v_src is-live=true format=time do-timestamp=false "
          << "caps=\"application/x-rtp,media=video,encoding-name=H264,payload=96,clock-rate=90000\" ! "
          << "rtpjitterbuffer latency=50 drop-on-latency=true ! " // 50ms 是平衡 MQTT 投递抖动与图传低延迟的极限值
